...
from asle.saml import authenticate, AuthenticationFailed, assume_role, AssumeRoleFailed, write_aws_credentials
from clickclick import Action, choice, error, AliasedGroup, info, print_table, OutputFormat

CONFIG_DIR_PATH = click.get_app_dir('asle')
CONFIG_FILE_PATH = os.path.join(CONFIG_DIR_PATH, 'asle.yaml')
...
def print_version(ctx, param, value):
    if not value or ctx.resilient_parsing:
        return
    click.echo('AWS SAML Login for EPAM {}'.format(asle.__version__))
    ctx.exit()
...
def cli(ctx, config_file, awsprofile):
    ...
    if not ctx.invoked_subcommand:
        if not data:
            raise click.UsageError('No profile configured. Use "asle create .." to create a new profile.')
        profile = None
        if 'global' in data:
...
def saml_login(user, url):
    ...
    saml_password = keyring.get_password('asle', ring_user)
    ...
    keyring.set_password('asle', ring_user, saml_password)
